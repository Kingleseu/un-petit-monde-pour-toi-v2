...
export default function App() {
  // If the URL path is /message, show the external guestbook form page
  const isGuestbookPage = window.location.pathname === '/message';

  return (
    <ContentProvider>
      {isGuestbookPage ? <GuestbookPage /> : <AppContentWrapper />}
    </ContentProvider>
  );
}
...
function GuestbookPage() {
  return (
    <div className="min-h-screen relative font-sans text-slate-100 overflow-x-hidden flex items-center justify-center">
      <Background />
    ...
    <div className="min-h-screen relative font-sans text-slate-100 overflow-x-hidden">
      ...
      {content.audioUrl && step > 0 && (
        <button 
          ...
          className="fixed bottom-4 right-16 z-50 p-2 opacity-50 hover:opacity-100 transition-opacity bg-white/10 hover:bg-[#D4AF37] rounded-full border border-white/10 hover:border-[#D4AF37] hover:text-[#050508] shadow-lg text-white" 
      ...
      <main className={`relative z-10 w-full min-h-screen flex items-center justify-center ${step > 0 ? 'pb-28 sm:pb-24' : ''}`}>
