...
export function GuestbookForm() {
  ...
  const form = content.guestbookForm;
  const formatText = (value = '') => value.replaceAll('{name}', name);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
          <div>
            <label className="block text-[10px] uppercase tracking-widest text-[#D4AF37] mb-2 font-bold">
              {formatText(form?.authorLabel)}
            </label>
            <input
              type="text"
              required
              placeholder={formatText(form?.authorPlaceholder)}
              value={newAuthor}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-widest text-[#D4AF37] mb-2 font-bold">
              {formatText(form?.wordLabel)}
            </label>
            <input
              type="text"
              required
              maxLength={20}
              placeholder={formatText(form?.wordPlaceholder)}
              value={newWord}
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-widest text-[#D4AF37] mb-2 font-bold">
              {formatText(form?.messageLabel)}
            </label>
            <textarea
              required
              placeholder={formatText(form?.messagePlaceholder)}
              value={newText}
          ...
          >
            {isSubmitting ? formatText(form?.submittingText) : formatText(form?.submitText)}
          </button>
        </form>
        ...
        {success && (
          ...
          >
            ...
            <h3 className="text-2xl font-serif text-white mb-3">{formatText(form?.successTitle)}</h3>
            <p className="text-white/60 text-sm max-w-sm leading-relaxed mb-6">
              {formatText(form?.successMessage)}
            </p>
            <button
            ...
            >
              {formatText(form?.writeAnotherText)}
            </button>
          </motion.div>
        )}
      </motion.div>
