...
const DEFAULT_WORDS = [
  "Solaire", "Unique", "Bienveillante", "Lumineuse", "Sincère", 
  "Drôle", "Généreuse", "Brillante", "Attentionnée", "Douce",
  "Inestimable", "Magique", "Inspirante", "Rayonnante", "Précieuse", 
  "Complice", "Formidable", "Extraordinaire", "Chaleureuse", "Fidèle",
  "Souriante", "Optimiste", "Sensible", "Créative", "Adorable"
];

function WordHeart({ messages }: { messages: { text: string; author: string; word?: string }[] }) {
  ...
  const fallbackWords = DEFAULT_WORDS.map(w => ({
    text: w,
    author: "L'univers"
  }));
  ...
  for (let layerIdx = 0; layerIdx < LAYERS.length; layerIdx++) {
    ...
    for (let i = 0; i < layer.count; i++) {
      ...
      if (layer.scale > 0) {
        ...
        const leftPercent = 50 + (x * 2.2 * layer.scale);
        const topPercent = 50 + (y * 2.2 * layer.scale) - 5;
        left = `${leftPercent}%`;
        top = `${topPercent}%`;
      }
  ...
  return (
    <div className="w-full flex flex-col items-center mt-16 py-10 border-t border-white/5">
      <div className="text-center mb-8">
        ...
        <h3 className="text-xl md:text-2xl font-serif text-white mb-2">Le Cœur des Mots</h3>
      ...
      <div className="relative w-full max-w-[min(420px,calc(100vw-3rem))] aspect-square bg-white/[0.01] border border-white/5 rounded-full flex items-center justify-center shadow-inner overflow-hidden mb-6">
        ...
        {positionedWords.map((w, idx) => (
          ...
          >
            <motion.span
              className={`cursor-pointer transition-all duration-300 select-none px-2 py-0.5 rounded hover:bg-white/5 break-keep whitespace-nowrap block ${w.sizeClass}`}
              animate={{ y: [0, -5, 0] }}
              transition={{
                duration: w.duration,
                repeat: Infinity,
              ...
              onMouseEnter={() => setHovered({ text: w.text, author: w.author })}
              onMouseLeave={() => setHovered(null)}
              onClick={() => setHovered({ text: w.text, author: w.author })}
            >
              {w.text}
            </motion.span>
          </motion.div>
          ...
          {hovered ? (
            ...
            >
              <p className="text-xs text-white/80">
                <span className="text-[#D4AF37] font-semibold">« {hovered.text} »</span> — décrit par <span className="font-semibold text-white">{hovered.author}</span>
              </p>
            </motion.div>
          ) : (
            <motion.p
            ...
            >
              Survolez ou touchez un mot pour voir qui l'a écrit...
            </motion.p>
          )}
        </AnimatePresence>
      </div>
    ...
    <div className="min-h-screen py-20 px-6 max-w-6xl mx-auto flex flex-col items-center">
      ...
      {allMessages.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm italic">Les messages de tes proches apparaîtront ici...</p>
        </motion.div>
      ) : (
      ...
      {/* Interactive Word Heart */}
      <WordHeart messages={allMessages} />
